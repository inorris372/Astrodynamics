...
if ~isempty(OutputFcn)
    ...
    output
    function
    with 'init' flag.Also check whether
...
acceptable
error
delta1 = max(abs(h2 * (fBhat - f * B))); % error
~ | Y - y |
delta2 = max(abs(h * (fBphat - f * Bp))); % error
~ | dot
{Y} - dot
{y} |
delta = max(delta1, delta2); % worst
case
error
...
if produce_output
    ...
    if have_outputFcn
        ...
        try
            OutputFcn
            {kk}(t, y(OutputSel), dy(OutputSel), 'done');
